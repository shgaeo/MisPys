...
##
def func_full_analysis(file_list, y0=0,a0=1,ignore_y0a0=False,flip_signal = [],
                       ws=6,sop=False,xText='free evolution $[\mu s]$',n_refs=2,n_signals=4,return_norm_data=2,add_minus_im=False):
    """
    ...
    for i in range(len(file_list)):
        ...
        xdat0,ndat,errdat = signal_normalized(file_path,mov_avg_windowSize=ws,sp=sop,xfact=1e6,xText=xText,
                                              titleText='full experiment',n_refs=n_refs,n_signals=n_signals)
        ## Normalized data 
    ...
    nd_re_1 = np.mean(np.array(ndata_0_list),axis=0)
    ed_re_1 = np.sqrt(np.sum(np.array(errdata_0_list)**2,axis=0))/len(file_list)
    nd_im_1 = np.mean(np.array(ndata_1_list),axis=0)
    ed_im_1 = np.sqrt(np.sum(np.array(errdata_1_list)**2,axis=0))/len(file_list)

    nv_short_re_1 = np.mean(np.array(nv_short_0_list),axis=0)
    nv_shortE_re_1 = np.sqrt(np.sum(np.array(nv_shortE_0_list)**2,axis=0))/(2*len(file_list))
    nv_short_im_1 = np.mean(np.array(nv_short_1_list),axis=0)
    nv_shortE_im_1 = np.sqrt(np.sum(np.array(nv_shortE_1_list)**2,axis=0))/(2*len(file_list))
...
##############################################
## Run simulation
## Using scrpit: kdq_2Lvls_interferometric.py
##
def func_simulation_ifft(u_sim,fact_τ, return_sim = 2, ideal_sim=False,
    ...
    """
    return_sim :  0=return simulation, 1=return simulation with u and (-u) parts, 2=return ifft of simulation
    """
    ##########################################################################################
    ##
    ## run simulation
    ...
    detu = -A*(1.0) #*(0.0) # 1/2 # MHz detuning high power gates
    ...
    if ideal_sim:
        solx,soly = ideal_experiment(u_sim,r0,θ,θ2,x_gate_angle=(2-fact_τ)*np.pi,δ_high=-A)
    else:
