...
import lens.fit_testing.lorentzian_fit_methods as lorfm
...
def make_gaussianwithoutoffset_model(prefix=None):
...
def make_gaussian_model(prefix=None):
    ...
    gaussian_model, params = make_gaussianwithoutoffset_model(prefix=prefix)
...
def make_gaussianlinearoffset_model(prefix=None):
    ...
    slope_model, params = lfm.make_slope_model(prefix)
    gaussian_model, params = make_gaussian_model(prefix)
...
def make_multiplegaussianoffset_model(no_of_functions=1):
    ...
    if no_of_functions == 1:
        multi_gaussian_model, params = make_gaussian_model()
    else:

        prefix = 'g0_'
        multi_gaussian_model, params = make_gaussianwithoutoffset_model(prefix=prefix)

        constant_model, params = lfm.make_constant_model()
        ...
        for ii in range(1, no_of_functions):
            ...
            multi_gaussian_model += make_gaussianwithoutoffset_model(prefix=prefix)[0]
            multi_gaussian_model.set_param_hint('{0}contrast'.format(prefix),
                                                expr='({0}amplitude/offset)*100'.format(prefix))
...
def make_gaussiandouble_model():
    ...
    return make_multiplegaussianoffset_model(no_of_functions=2)
...
def make_gaussiantriple_model():
    ...
    return make_multiplegaussianoffset_model(no_of_functions=3)

#####################
# 2D gaussian model #
#####################

def make_twoDgaussian_model(prefix=None):
...
def make_gaussian_fit(x_axis, data, estimator, units=None, add_params=None, **kwargs):
    ...
    mod_final, params = make_gaussian_model()
    ...
    try:
        ...
        print('The 1D gaussian peak fit did not work. Error '
...
def estimate_gaussian_peak(x_axis, data, params):
...
def estimate_gaussian_dip(x_axis, data, params):
    ...
    error, params_ret = estimate_gaussian_peak(x_axis,
                                                          data_negative,
                                                          params_peak
                                                          )
...
def make_gaussianlinearoffset_fit(x_axis, data, estimator, units=None, add_params=None, **kwargs):
    ...
    mod_final, params = make_gaussianlinearoffset_model()
...
def estimate_gaussianlinearoffset_peak(x_axis, data, params):
    ...
    res_ordinary_gauss = make_gaussian_fit(
        x_axis=x_axis,
        data=data,
        units=None,
        estimator=estimate_gaussian_peak
    )

    # subtract the result and perform again a linear fit:
    data_subtracted = data - res_ordinary_gauss.best_fit

    res_linear = lfm.make_linear_fit(
        x_axis=x_axis,
        data=data_subtracted,
        estimator=lfm.estimate_linear
    )
...
def make_gaussiandouble_fit(x_axis, data, estimator,
                            units=None,
                            add_params=None,
    ...
    model, params = make_multiplegaussianoffset_model(no_of_functions=2)
    ...
    try:
        ...
        result = model.fit(data, x=x_axis, params=params, **kwargs)
        print('The double gaussian dip fit did not work: {0}'.format(
            result.message))
...
def estimate_gaussiandouble_peak(x_axis, data, params,
    ...
    mod_lor, params_lor = lorfm.make_multiplelorentzian_model(no_of_functions=2)

    error, params_lor = lorfm.estimate_lorentziandouble_dip(x_axis=x_axis,
                                                     data=-data,
                                                     params=params_lor,
...
def estimate_gaussiandouble_dip(x_axis, data, params,
                               threshold_fraction=0.4, minimal_threshold=0.1,
                               sigma_threshold_fraction=0.2):
...
def make_twoDgaussian_fit(xy_axes, data, estimator, units=None, add_params=None, **kwargs):
    ...
    gaussian_2d_model, params = make_twoDgaussian_model()
    ...
    try:
        ...
        result = gaussian_2d_model.fit(data, x=xy_axes, params=params, **kwargs)
        print('The 2D gaussian fit did not work: {0}'.format(
                       result.message))

    return result

def estimate_twoDgaussian(x_axis, y_axis, data, params):
    ...
    for var in parameters:
        ...
        if not isinstance(var, (frozenset, list, set, tuple, np.ndarray)):
            print('Given parameter is not an array.')
            amplitude = 0.
            center_x = 0.
            center_y = 0.
            sigma_x = 0.
...
def estimate_twoDgaussian_MLE(x_axis, y_axis, data, params):
