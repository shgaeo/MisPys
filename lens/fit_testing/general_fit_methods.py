...
def _substitute_params(initial_params, update_params=None):
    ...
    # Check the case for an OrderedDict or dict parameter:
        ...
        log.error('The type of the passed update_params object <{0}> is '
...
def create_fit_string(result, model, units=None, decimal_digits_value_given=None,
    ...
    for variable in model.param_names:
        ...
        try:
            ...
            # log.warning('No unit given for parameter {}, setting unit '
...
def _search_end_of_dip(direction, data, peak_arg, start_arg, end_arg, sigma_threshold, minimal_threshold, make_prints):
    """
...
def make_lorentzian_model(prefix=None):
    ...
    For further information see same function in lorentzian_fit_methods.py
    """
...
def make_lorentzianwithoutoffset_model(prefix=None):
    ...
    if not isinstance(prefix, str) and prefix is not None:
        ...
        lorentz_model = lmfit.Model(physical_lorentzian, independent_vars='x')
    else:
        lorentz_model = lmfit.Model(physical_lorentzian, independent_vars='x', prefix=prefix)
...
def _check_1D_input(x_axis, data, params):
    ...
    for var in parameters:
        if not isinstance(var, (frozenset, list, set, tuple, np.ndarray)):
            log.error('Given parameter is no array.')
            error = -1
        elif len(np.shape(var)) != 1:
            log.error('Given parameter is no one dimensional array.')
            error = -1
    if not isinstance(params, Parameters):
        log.error('Parameters object is not valid in estimate_gaussian.')
        error = -1
